...
An app that just wants the design language — not the React components — takes
the tokens alone:

```css
@import '@jemmy8oy-northstar/design-system/tokens.css';
```

`tokens.css` is the primitives → semantic roles → themes → base layers with no
component CSS, generated by `npm run build`. It is the artefact to vendor for a
tokens-only ("L1") adoption. Which app is on which theme, at which rung, is
tracked in **[docs/consumers.md](docs/consumers.md)** — the adoption PR updates
that register in the same PR.
